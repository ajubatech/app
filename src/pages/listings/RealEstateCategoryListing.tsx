...
export default function RealEstateCategoryListing() {
  ...
  const loadProperties = async () => {
    try {
      ...
      // Apply filters for property type categories
      if (category && category !== 'all' && 
          (category === 'house' || category === 'apartment' || category === 'townhouse' || category === 'land')) {
        query = query.eq('real_estate_metadata.property_type', category);
      }
      ...
      // Filter by sale type categories after fetching data
      if (category && category !== 'all' && 
          (category === 'for-sale' || category === 'for-rent' || category === 'sold')) {
        filteredData = filteredData.filter(item => 
          item.metadata?.sale_type === category.replace('-', '_')
        );
      }
  ...
  const getCategoryTitle = () => {
    if (!category || category === 'all') return 'All Real Estate';
    ...
    return `Real Estate - ${formattedCategory}`;
  };
